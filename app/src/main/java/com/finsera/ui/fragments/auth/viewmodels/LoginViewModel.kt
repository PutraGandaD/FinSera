...
class LoginViewModel(
                    ...
                    when (result) {
                        is Resource.Loading -> {
                            _loginScreenUIState.update { uiState ->
                                uiState.copy(isLoading = true, errorMessage = null, isUserLoggedIn = false)
                            }
                        }
                        is Resource.Success -> {
                            _loginScreenUIState.update { uiState ->
                                uiState.copy(isLoading = false, errorMessage = result.data.toString(), isUserLoggedIn = true)
                            }
                            SharedPreferenceManager.saveBoolean(USER_LOGGED_IN_STATUS, true)
                        }
                        is Resource.Error -> {
                            _loginScreenUIState.update { uiState ->
                                uiState.copy(isLoading = false, errorMessage = result.message, isUserLoggedIn = false)
                            }
                        }
                    }
                }
    ...
    fun checkUserLoggedInStatus()  {
        val value = SharedPreferenceManager.getBoolean(USER_LOGGED_IN_STATUS, false)
        _userLoggedInStatus.value = value
    }
